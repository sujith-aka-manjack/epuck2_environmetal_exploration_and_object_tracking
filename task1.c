...
int main(void)
{
    ...
    while (1)
    {
        	...
        	if(obs_side == 1)	//If obstacle is at right, turn left till the right prox doesnt sense any obstacle
           	...
           	else
           	{
           		...
           		while(prox_max>obj_true)
           		{
           			...
           			left_motor_set_speed(250);
           			right_motor_set_speed(-250);
           			prox_max = get_calibrated_prox(6);
        ...
        else		// If no obstacle then continue in straight line
        {
        	...
        	left_motor_set_speed(475);
        	right_motor_set_speed(475);
        }
        //To prevent only wall tracing, if robot makes 6 consecutive turns in the same dir, then rotate robot
        if(count_left>6)
        {
        	clear_leds();
